...
- Frames are found by the bare film between them rather than by their edges. A gap holds no picture, so it reads flat down the sensor whatever the film is, and a picture does not: the strip is fitted with one length and one pitch shared by every frame, which is what a fixed camera window and a fixed transport advance produce. `scan::strip` replaces `boundaries::detect`.
- **Breaking:** `framing::discover`, `framing::discover_with` and Python's `discover_frames` no longer take the film's polarity, and `boundaries::detect` and `boundaries::Detected` are gone. Which way the film reads no longer tells anything apart on a thumbnail.
- **Breaking:** `boundaries::locate` takes no expected length and answers the picture rather than an `Option`, and `boundaries::Picture` is gone: `Session::autoexpose_frame` and `autoexpose_frame_with` take an `Option<Polarity>`. A pass that opened inside the picture answers from column 0, which is how a caller tells that the pass clipped the frame.
...
- A frame the metering pass measures is placed by the bare film either side of the picture rather than by the strength of its edges. On an LS-50 the edge finder put one frame's picture 440 addresses - 2.8 mm - further into its pass than it was and the correction moved the film that far, so a frame the strip fit had placed within a millimeter came back with its picture running off the head of the pass and the next frame's showing at the tail. Bare film is the brightest thing a negative holds and the densest a slide does, whatever was photographed, so the picture is what lies inside the run of it at each end. Measured as columns of bare film either side of the picture, of the 29 a 135 frame has to share: that frame scanned 0/6, and the six frame strip it came from now scans 15/14, 18/11, 17/11, 15/14, 25/6 and 24/7, every picture whole.
- A pass that opened inside the picture is placed from the film behind the picture instead, a frame's length back, which is the case that most needs moving and the one with no film in front of it to measure. `take_picture_start` is signed for it: a picture that starts before the pass did is where the old reading had nothing to say.
- A pass reads its levels against its own full scale rather than 16 bits. A metering pass is read before anything stretches it, so a 14-bit unit put every level in the bottom quarter of the range and nothing in the pass looked like bare film.
- Moving a frame back past the start of the axis kept its length: the two edges were clamped separately, which left the frame shorter than the format and the pass looking for a frame that was not there.
- A frame is scanned at the format the caller asked for. The perforation path replaced it with the length the detection measured off the picture, and then took the film ahead of the frame from that length's slack, so a picture measured short both cut the scan and moved the film, twice over. Nothing but the middle of the picture now decides where the pass opens: it is half a scannable range ahead of it, which is the entry that puts the frame in the middle of the pass.
...
- The thumbnail line is measured against the unit's own perforation table rather than computed from the thumbnail resolution it reports. An LS-50 reports 97 dpi, so a line was taken as 4000/97 = 41 addresses where the film moves 42.0, and every frame built from thumbnail columns came out about a millimeter short over its length. 2-11-8's records are one line's absolute position in perforations of 4.7498 mm, which measures it directly.
- A frame on a perforation-framed unit is centered in its pass, where before it opened on the first line of the picture with the whole slack of the range after it and no film in front of it to find the leading edge against. The frame table takes the whole entry back by half the slack, top and record together, and the metering pass then measures where the picture is so the fine pass can move it the rest of the way. Measured on an LS-50 with the SA-21: 0 and 248 addresses of film either side of a 135 frame, now 104 and 128.
- The correction that moves a frame onto the range aims at the middle of it rather than the top, moves the whole rectangle rather than its top alone, and moves in either direction. Any measurement at all used to move every frame to the top of the range, and moving the top without the bottom left the pass looking for a frame short by however far it moved.
